...
type core_type =
  | Ptyp_var of string (** Type variables ['a]. *)
  | Ptyp_arrow of core_type * core_type (** Function types [T1 -> T2]. *)
  | Ptyp_tuple of core_type list (** Product (or "tuple") types. *)
  ...
  | Ptyp_variant of row (** Polymorphic variants [ [ ... ] ] *)
  | Ptyp_mu of string * core_type (** Equi-recursive types [ mu 'a. T ] *)
  | Ptyp_where of core_type * string * core_type
  (** where-binding [ T where 'a = T ] *)
[@@deriving sexp_of]

and row = row_field list * closed_flag [@@deriving sexp_of]

and row_field = Row_tag of string * core_type option (** [ `A ]
                                                         [ `A of T ] *)
[@@deriving sexp_of]
...
type pattern =
  | Ppat_any (** [_]. *)
  | Ppat_var of string (** [x]. *)
  | Ppat_alias of pattern * string (** [P as x]. *)
  | Ppat_const of constant (** [c]. e.g. [1, true, ()]. *)
  | Ppat_tuple of pattern list (** [(P1, ..., Pn)]. Invariant n >= 2. *)
  | Ppat_construct of string * (string list * pattern) option (** [C <P>]. *)
  | Ppat_variant of string * pattern option (** [`A <P>]. *)
...
type expression =
  ...
  | Pexp_app of expression * expression (** Function application [E1 E2]. *)
  ...
  | Pexp_record of (string * expression) list (** [{l1 = E1; ...; ln = En}] *)
  | Pexp_field of expression * string (** [E.l] *)
  ...
  | Pexp_variant of string * expression option (** [`A <E>] *)
[@@deriving sexp_of]
...
type type_declaration =
  { ptype_name : string (* Type name [t] *)
  ; ptype_params : string list (** Parameters: [('a1, .., 'an) t] *)
  ; ptype_kind : type_decl_kind (** Declaration kinds: record or variant *)
  }
[@@deriving sexp_of]
...
and label_declaration =
  { plabel_name : string (** Label name [l] *)
  ; plabel_betas : string list
  (** Label betas, used for semi-explicit first-class polymorphism *)
  ; plabel_arg : core_type (** Label argument type *)
  }
[@@deriving sexp_of]

(** Constructor declaration: [C of 'b1 .. 'bm. T constraint E] *)
and constructor_declaration =
  { pconstructor_name : string (** Constructor name [C] *)
  ; pconstructor_arg : constructor_argument option (** Constructor argument *)
...
type type_exception = { ptyexn_constructor : extension_constructor }
[@@derving sexp_of]

(** Extensions [type t += ..] *)
type type_extension =
  { ptyext_name : string (* Type name [t] *)
  ; ptyext_params : string list (** Parameters: [('a1, .., 'an) t] *)
...
type structure_item =
  ...
  | Pstr_primitive of value_description (** External primitive descriptions *)
  | Pstr_type of type_declaration list
  (** Type declarations [type t1 = ... and ... tn = ...] *)
  | Pstr_typext of type_extension (** Type extensions [type t += ...] *)
  | Pstr_exception of type_exception (** Exception [exception C <of T>] *)
[@@deriving sexp_of]
